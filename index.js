...
// end include: /tmp/tmph1nh906r.js
// include: /tmp/tmpgbsjcdcl.js

    // All the pre-js content up to here must remain later on, we need to run
    // it.
    if ((typeof ENVIRONMENT_IS_WASM_WORKER != 'undefined' && ENVIRONMENT_IS_WASM_WORKER) || (typeof ENVIRONMENT_IS_PTHREAD != 'undefined' && ENVIRONMENT_IS_PTHREAD) || (typeof ENVIRONMENT_IS_AUDIO_WORKLET != 'undefined' && ENVIRONMENT_IS_AUDIO_WORKLET)) Module['preRun'] = [];
    var necessaryPreJSTasks = Module['preRun'].slice();
  // end include: /tmp/tmpgbsjcdcl.js
// include: /tmp/tmpwqbu415t.js

    if (!Module['preRun']) throw 'Module.preRun should exist because file support used it; did a pre-js delete it?';
    necessaryPreJSTasks.forEach((task) => {
      if (Module['preRun'].indexOf(task) < 0) throw 'All preRun tasks that exist before user pre-js code should remain after; did you replace Module or modify Module.preRun?';
    });
  // end include: /tmp/tmpwqbu415t.js
...
var ASM_CONSTS = {
  145528: () => { if (document.fullscreenElement) return 1; },  
 145574: () => { return document.getElementById('canvas').width; },  
 145626: () => { return parseInt(document.getElementById('canvas').style.width); },  
 145694: () => { document.exitFullscreen(); },  
 145721: () => { setTimeout(function() { Module.requestFullscreen(false, false); }, 100); },  
 145794: () => { if (document.fullscreenElement) return 1; },  
 145840: () => { return document.getElementById('canvas').width; },  
 145892: () => { return screen.width; },  
 145917: () => { document.exitFullscreen(); },  
 145944: () => { setTimeout(function() { Module.requestFullscreen(false, true); setTimeout(function() { canvas.style.width="unset"; }, 100); }, 100); },  
 146077: () => { return window.innerWidth; },  
 146103: () => { return window.innerHeight; },  
 146130: () => { if (document.fullscreenElement) return 1; },  
 146176: () => { return document.getElementById('canvas').width; },  
 146228: () => { return parseInt(document.getElementById('canvas').style.width); },  
 146296: () => { if (document.fullscreenElement) return 1; },  
 146342: () => { return document.getElementById('canvas').width; },  
 146394: () => { return screen.width; },  
 146419: () => { return window.innerWidth; },  
 146445: () => { return window.innerHeight; },  
 146472: () => { if (document.fullscreenElement) return 1; },  
 146518: () => { return document.getElementById('canvas').width; },  
 146570: () => { return screen.width; },  
 146595: () => { document.exitFullscreen(); },  
 146622: () => { if (document.fullscreenElement) return 1; },  
 146668: () => { return document.getElementById('canvas').width; },  
 146720: () => { return parseInt(document.getElementById('canvas').style.width); },  
 146788: () => { document.exitFullscreen(); },  
 146815: ($0) => { document.getElementById('canvas').style.opacity = $0; },  
 146873: () => { return screen.width; },  
 146898: () => { return screen.height; },  
 146924: () => { return window.screenX; },  
 146951: () => { return window.screenY; },  
 146978: ($0) => { navigator.clipboard.writeText(UTF8ToString($0)); },  
 147031: ($0) => { document.getElementById("canvas").style.cursor = UTF8ToString($0); },  
 147102: () => { document.getElementById('canvas').style.cursor = 'none'; },  
 147159: ($0, $1, $2, $3) => { try { navigator.getGamepads()[$0].vibrationActuator.playEffect('dual-rumble', { startDelay: 0, duration: $3, weakMagnitude: $1, strongMagnitude: $2 }); } catch (e) { try { navigator.getGamepads()[$0].hapticActuators[0].pulse($2, $3); } catch (e) { } } },  
 147415: ($0) => { document.getElementById('canvas').style.cursor = UTF8ToString($0); },  
 147486: () => { if (document.fullscreenElement) return 1; },  
 147532: () => { return window.innerWidth; },  
 147558: () => { return window.innerHeight; },  
 147585: () => { if (document.pointerLockElement) return 1; },  
 147632: ($0, $1, $2, $3, $4) => { if (typeof window === 'undefined' || (window.AudioContext || window.webkitAudioContext) === undefined) { return 0; } if (typeof(window.miniaudio) === 'undefined') { window.miniaudio = { referenceCount: 0 }; window.miniaudio.device_type = {}; window.miniaudio.device_type.playback = $0; window.miniaudio.device_type.capture = $1; window.miniaudio.device_type.duplex = $2; window.miniaudio.device_state = {}; window.miniaudio.device_state.stopped = $3; window.miniaudio.device_state.started = $4; miniaudio.devices = []; miniaudio.track_device = function(device) { for (var iDevice = 0; iDevice < miniaudio.devices.length; ++iDevice) { if (miniaudio.devices[iDevice] == null) { miniaudio.devices[iDevice] = device; return iDevice; } } miniaudio.devices.push(device); return miniaudio.devices.length - 1; }; miniaudio.untrack_device_by_index = function(deviceIndex) { miniaudio.devices[deviceIndex] = null; while (miniaudio.devices.length > 0) { if (miniaudio.devices[miniaudio.devices.length-1] == null) { miniaudio.devices.pop(); } else { break; } } }; miniaudio.untrack_device = function(device) { for (var iDevice = 0; iDevice < miniaudio.devices.length; ++iDevice) { if (miniaudio.devices[iDevice] == device) { return miniaudio.untrack_device_by_index(iDevice); } } }; miniaudio.get_device_by_index = function(deviceIndex) { return miniaudio.devices[deviceIndex]; }; miniaudio.unlock_event_types = (function(){ return ['touchend', 'click']; })(); miniaudio.unlock = function() { for(var i = 0; i < miniaudio.devices.length; ++i) { var device = miniaudio.devices[i]; if (device != null && device.webaudio != null && device.state === window.miniaudio.device_state.started) { device.webaudio.resume().then(() => { Module._ma_device__on_notification_unlocked(device.pDevice); }, (error) => {console.error("Failed to resume audiocontext", error); }); } } miniaudio.unlock_event_types.map(function(event_type) { document.removeEventListener(event_type, miniaudio.unlock, true); }); }; miniaudio.unlock_event_types.map(function(event_type) { document.addEventListener(event_type, miniaudio.unlock, true); }); } window.miniaudio.referenceCount += 1; return 1; },  
 149790: () => { if (typeof(window.miniaudio) !== 'undefined') { window.miniaudio.referenceCount -= 1; if (window.miniaudio.referenceCount === 0) { delete window.miniaudio; } } },  
 149954: () => { return (navigator.mediaDevices !== undefined && navigator.mediaDevices.getUserMedia !== undefined); },  
 150058: () => { try { var temp = new (window.AudioContext || window.webkitAudioContext)(); var sampleRate = temp.sampleRate; temp.close(); return sampleRate; } catch(e) { return 0; } },  
 ...
 153057: ($0) => { return miniaudio.get_device_by_index($0).webaudio.sampleRate; },  
 153123: ($0) => { var device = miniaudio.get_device_by_index($0); if (device.scriptNode !== undefined) { device.scriptNode.onaudioprocess = function(e) {}; device.scriptNode.disconnect(); device.scriptNode = undefined; } if (device.streamNode !== undefined) { device.streamNode.disconnect(); device.streamNode = undefined; } device.webaudio.close(); device.webaudio = undefined; device.pDevice = undefined; },  
 153516: ($0) => { miniaudio.untrack_device_by_index($0); },  
 153559: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.resume(); device.state = miniaudio.device_state.started; },  
 153684: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.suspend(); device.state = miniaudio.device_state.stopped; }
};
